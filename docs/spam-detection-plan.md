...
model votes → emit; the judge is Claude Haiku via the Anthropic Messages API since 2026-09-05, after co/core dropped the served model id twice — `REVIEW_PROVIDER=openai` still reaches any chat-completions endpoint), and the policy line evolved to **composed vs
