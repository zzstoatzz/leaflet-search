...
| tool | description |
|------|-------------|
| `search` | hybrid search by default; narrow with `tag`, `platform`, `since`, `author`, each binding in every mode; `mode` picks keyword or semantic |
...
- `contentLength` — the indexed text's length, 0 only when the index has no
  copy. a few hundred characters is a linkblog stub (a pull-quote indexed as
  the whole document), not primary writing.
...
search("appview takedowns", tag="moderation") → hybrid, and every result carries the tag
